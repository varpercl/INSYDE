...
void TrainingSetDialog::onInputsNormalizationTypeChanged(ITrainingSet::NormalizationType nt)
{
	switch(nt){
		...
		case ITrainingSet::Tanh:{
			dblInputsAmpValue = inw->getAmplitudeValue();
			dblInputsElongValue = inw->getElongationValue();
			normalizedTS->setInputs(ts->getNormalizedInputsTanh(dblInputsAmpValue, dblInputsElongValue));
			break;
		}
		case ITrainingSet::Sigmoid:{
			dblInputsAmpValue = inw->getAmplitudeValue();
			dblInputsElongValue = inw->getElongationValue();
			normalizedTS->setInputs(ts->getNormalizedInputsSigmoid(dblInputsAmpValue, dblInputsElongValue));
			break;
		}
		case ITrainingSet::MeanDistance:
			normalizedTS->setInputs(ts->getNormalizedInputsMeanDistance());
			break;
	}
...
void TrainingSetDialog::onTargetsNormalizationTypeChanged(ITrainingSet::NormalizationType nt)
{
	...
	switch(nt){
		...
		case ITrainingSet::Tanh:
			*normalizedTS = ts->getNormalizedTanh();
			break;
		case ITrainingSet::Sigmoid:
			*normalizedTS = ts->getNormalizedSigmoid();
			break;
		case ITrainingSet::MeanDistance:
			*normalizedTS = ts->getNormalizedMeanDistance();
			break;
	}
	patternTable->setTrainingSet(normalizedTS);
}
