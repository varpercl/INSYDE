		...
		if(btn->isChecked()){
			if(visor->itemAt(event->pos()) == NULL){
				if(btn == ui->btnAddADALINE){
					...
					QString path = QFileDialog::getOpenFileName(this, "Abrir archivo", "", "Mapa de bits (*.bmp);; Formato JPEG (*.jpg);; Portable Network Graphics (*.png);; Formato de Intercambio de Graficos (*.gif)");
					...
					giee = new GraphicImageEffectElement();
