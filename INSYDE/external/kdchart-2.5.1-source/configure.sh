...
[ -z "$QTDIR" ] && die "You need QTDIR defined, or either qmake or qmake-qt5 or qmake-qt4 in the PATH"
...
"$QTDIR"/bin/qmake ${SPEC} "$SOURCE_DIR"/$product.pro -recursive $QMAKE_ARGS "${PRODUCT}_BASE=`pwd`" || die "qmake failed"
