...
QVariant TrainingResult::data(const QModelIndex &index, int role) const
{
	...
	if(row < rowCount() && row >= 0 && col >= 0 && col < columnCount()){
		if(col == 0 || col == 2){
			return row;
		}else if(col == 1 && visibleMSE){
			return mseHistory[row];
		}else if(col == 3 && visibleRMSE){
...
void TrainingResult::setMSEVisible(bool hi)
{
	visibleMSE = hi;
}

void TrainingResult::setRMSEVisible(bool hi)
{
	visibleRMSE = hi;
}

bool TrainingResult::isEmpty() const
{
	return epochs == 0;
}

void TrainingResult::init()
{
	visibleMSE = false;
	visibleRMSE = false;
	clear();
