...
StatusAnimationControl::StatusAnimationControl(QWidget *parent) : QFrame(parent)
{
    iniciarStatusAnimation();
}

void StatusAnimationControl::iniciarStatusAnimation()
{
    status = Stopped;
...
void StatusAnimationControl::playPauseClick()
{
    Status past = status;
    switch(status){
        case Stopped:
...
void StatusAnimationControl::stopClick()
{
    Status past = status;
    status = Stopped;
    emit stop();
...
Status StatusAnimationControl::getCurrentStatus()
{
    return status;
}
