...
class ITrainingSet{
	...
	public:
		 ...
		 * @return
		 */
		virtual int getInputsSize() const = 0;
		 ...
		 * @return
		 */
		virtual int getTargetsSize() const = 0;
		 ...
		 * \param index
		 */
		virtual void removePattern(int index) = 0;

		/*!
		 * \brief setInputs Asigna el subconjunto de entradas \code{inputs} al conjunto de entrenamiento.
		 *
		 * \note Este metodo no modifica el numero de patrones de conjunto de entrenamiento. Solamente se tomaran el
		 * numero de patrones de entrada establecido por la propiedad \code{patternCount} que tiene actualmente
		 * el conjunto de entrenamiento.
		 * Si el conjunto de entradas que se asignara tiene un numero de patrones distinto al actualmente asignado al
		 * conjunto de entrenamiento y desea que el mismo tenga un numero de patrones igual al conjunto de entrada que
		 * se quiere asignar debe llamar explicitamente el metodo \code{setPatternCount()}
		 *
		 * En la implementacion original los valores de entrada son del tipo \code{double}.
		 *
		 * \param inputs Una matriz MxN que contiene el conjunto de entradas que se asignara al conjunto de entrenamiento
		 * \param size Indica el tamaño del patron de entrada (numero de columnas) que tendra el conjunto de entrenamiento
		 *
		 * \sa ITrainingSet::setInputs()
		 */
		virtual void setInputs(const vector<vector<double> > &inputs, int size) = 0;

		/*!
		 * \brief setInputs Este metodo es una sobrecarga para soportar valores de tipo \code{int}.
		 *
		 * \overload setInpus()
		 *
		 * \param inputs
		 ...
		 * \sa ITrainingSet::setInputs()
		 */
		virtual void setInputs(const vector<vector<int> > &inputs, int size) = 0;
		 ...
		 * \param targets
		 */
		virtual void setTargets(const vector<vector<double> > &targets, int size) = 0;
		 ...
		 * \param targets
		 */
		virtual void setTargets(const vector<vector<int> > &targets, int size) = 0;
		...
		virtual void normalizeTanh(double inAmp = 1, double inElong = 1, double tarAmp = 1, double tarElong = 1) = 0;
		virtual void normalizeInputsTanh(double amp = 1, double elong = 1) = 0;
		virtual void normalizeTargetsTanh(double amp = 1, double elong = 1) = 0;

		virtual TrainingSet getNormalizedTanh(double inAmp = 1, double inElong = 1, double tarAmp = 1, double tarElong = 1) = 0;

		virtual vector<vector<double> > getNormalizedInputsTanh(double amp = 1, double elong = 1) = 0;
		virtual vector<vector<double> > getNormalizedTargetsTanh(double amp = 1, double elong = 1) = 0;

		virtual void normalizeSigmoid(double inAmp = 1, double inElong = 1, double tarAmp = 1, double tarElong = 1) = 0;
		virtual void normalizeInputsSigmoid(double amp = 1, double elong = 1) = 0;
		virtual void normalizeTargetsSigmoid(double amp = 1, double elong = 1) = 0;

		virtual TrainingSet getNormalizedSigmoid(double inAmp = 1, double inElong = 1, double tarAmp = 1, double tarElong = 1) = 0;
		virtual vector<vector<double> > getNormalizedInputsSigmoid(double amp = 1, double elong = 1) = 0;
		virtual vector<vector<double> > getNormalizedTargetsSigmoid(double amp = 1, double elong = 1) = 0;

		virtual void normalizeMeanDistance() = 0;
		virtual void normalizeInputsMeanDistance() = 0;
		virtual void normalizeTargetsMeanDistance() = 0;

		virtual TrainingSet getNormalizedMeanDistance() = 0;
		virtual vector<vector<double> > getNormalizedInputsMeanDistance() = 0;
		virtual vector<vector<double> > getNormalizedTargetsMeanDistance() = 0;

//		virtual void normalizeSigmoid(double *inThreshold, double *tarThreshold) = 0;

		/*!
		 * \brief normalize Realiza una normalizacion simultanea de entradas y salidas segun los parametros \code{in} y \code{tn} respectivamente.
		 * \param in Indica la normalizacion que se usara para las entradas
		 * \param tn Indica la normalizacion que se usara para las salidas
		 */
		virtual void normalize(Normalization in, Normalization tn) = 0;
		 ...
		 * \param in Normalizacion que se empleara para las entradas
		 * \param tn Normalizacion que se empleara para las salidas
		 * \return Devuelve un nuevo objeto \code{TrainingSet} que es una copia fiel donde el conjunto de entrenamiento original
		 * sera el ya normalizado, mientras que el conjunto de entrenamiento normalizado tendra como normalizacion \code{Nothing}
		 *
		 * Cabe destacar que cada objeto \code{TrainingSet} contiene un conjunto con los valores originales y un conjunto con los valores normalizados.
		 * Para mayor informacion vea la definicion de la clase \code{TrainingSet}
		 *
		 * \see TrainingSet
		 */
		virtual TrainingSet getNormalized(Normalization in, Normalization tn) const = 0;
		 ...
		 *
		 * \param nt Tipo de normalizacion que se asignara
		 *
		 * NOTE: este metodo fue sustituido por \code{setNormalization(Normalization)} que permite suministrar mas informacion acerca de
		 * la normalizacion que se asignara.
		 *
		 */
//		virtual void setNormalizationType(NormalizationType nt) = 0;
		...
		virtual void setInputsNormalization(const Normalization &nor);
		virtual Normalization getInputsNormalization() const;

		virtual void setTargetsNormalization(const Normalization &nor);
		virtual Normalization getTargetsNormalization() const;

	protected:
		Normalization inputsNormalization, targetsNormalization;
};
