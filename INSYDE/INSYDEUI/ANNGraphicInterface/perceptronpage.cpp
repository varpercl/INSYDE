...
{
	WeightsEditorDialog *wed = new WeightsEditorDialog(perceptronDisplay->getPerceptron());
	if(wed->exec() == QDialog::Accepted){
		perceptronDisplay->setWeightsVector(wed->getWeights());
//		sp->setWeights(wed->getWeights());
	}
	delete wed;
