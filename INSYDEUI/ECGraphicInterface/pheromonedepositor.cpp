...
void PheromoneDepositor::placePheromoneItem(int intensity)
{
	...
	int numItems;
//    QGraphicsObject *obj;
	int tmpX = CommonFunctions::roundedValue(ag->x(), gridSpace);
	int tmpY = CommonFunctions::roundedValue(ag->y(), gridSpace);
...
{
	return lastPheromoneItem ? CommonFunctions::distanceBetweenPoints(lastPheromoneItem->pos(), getAgent()->pos()) : NAN;
}
...
//    QGraphicsObject *obj;
	int tmpX = CommonFunctions::roundedValue(ag->x(), gridSpace),
			tmpY = CommonFunctions::roundedValue(ag->y(), gridSpace);
	bool putNewPheromone = true;
